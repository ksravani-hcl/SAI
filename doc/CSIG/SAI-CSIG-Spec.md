...
### 5.3 Enable CSIG tag update for CSIG packets with d bit set
Per UEC CSIG Spec, when d bit is set, subsequent switches MUST not update the signal value. There might be an exception to this and switch may be requested to updated the signal value for signals that are not depenendent on TRIM queue like ABW and ABW/C.

Enabling of signal update is done via TAM INT object binding to the ACL match rule where rule includes match on d bit.

- match <﻿SAI_ACL_ENTRY_ATTR_FIELD_IN_PORT | SAI_ACL_ENTRY_ATTR_FIELD_CSIG_COMPACT_ETHER_TYPE | AI_ACL_ENTRY_ATTR_FIELD_CSIG_SIGNAL_TYPE> action <﻿SAI_ACL_ENTRY_ATTR_ACTION_TAM_INT_OBJECT>

### 5.4 Supporting CSIG Stats
...
```
sai_s32_list_t        csig_tag_enum_list;

// Query CSIG tag support for compact and/or wide tag
sai_query_attribute_enum_values_capability(switch_id, SAI_OBJECT_TYPE_TAM_INT,  ﻿
﻿SAI_TAM_INT_ATTR_INT_TYPE,  &csig_tag_enum_list);

// Query CSIG signals supported
sai_query_attribute_enum_values_capability(switch_id, SAI_OBJECT_TYPE_TAM_INT,  ﻿
SAI_TAM_INT_ATTR_CSIG_SIGNAL_TYPE,  &csig_tag_enum_list);
...
get_switch_attribute(switch_id,attr_cnt,  ﻿
